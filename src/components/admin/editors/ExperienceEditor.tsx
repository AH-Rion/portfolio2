...
export default function ExperienceEditor() {
  ...
  const load = async () => {
    ...
    try {
      ...
      console.error('[ExperienceEditor] load failed:', err);
      toast('Could not load experience.', 'error');
      setItems([]);
    } finally {
      setLoading(false);
    }
  ...
  const save = async () => {
    ...
    try {
      ...
      if (error) {
        console.error('[ExperienceEditor] insert/update error:', error.message);
      ...
      console.error('[ExperienceEditor] save failed:', err);
      toast('Could not save entry. Please try again.', 'error');
    } finally {
      setSaving(false);
    }
  };
  ...
  const remove = async (id: string) => {
    try {
      ...
      if (error) {
        console.error('[ExperienceEditor] delete error:', error.message);
        return toast(error.message, 'error');
      }
      toast('Entry deleted.', 'success');
      await load();
    } catch (err) {
      console.error('[ExperienceEditor] delete failed:', err);
  ...
  return (
    <div>
      ...
      {(formOpen || (items.length === 0 && !loading)) && (
