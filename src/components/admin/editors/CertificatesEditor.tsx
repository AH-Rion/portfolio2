...
export default function CertificatesEditor() {
  ...
  const load = async () => {
    ...
    try {
      ...
      console.error('[CertificatesEditor] load failed:', err);
      toast('Could not load certificates.', 'error');
      setItems([]);
    } finally {
      setLoading(false);
    }
  ...
  const save = async () => {
    ...
    try {
      ...
      if (error) {
        console.error('[CertificatesEditor] insert/update error:', error.message);
      ...
      console.error('[CertificatesEditor] save failed:', err);
      toast('Could not save certificate. Please try again.', 'error');
    } finally {
      setSaving(false);
    }
  };
  ...
  const remove = async (id: string) => {
    try {
      ...
      if (error) {
        console.error('[CertificatesEditor] delete error:', error.message);
        return toast(error.message, 'error');
      }
      toast('Certificate deleted.', 'success');
      await load();
    } catch (err) {
      console.error('[CertificatesEditor] delete failed:', err);
  ...
  const move = async (c: Certificate, dir: -1 | 1) => {
    try {
      ...
      console.error('[CertificatesEditor] reorder failed:', err);
      toast('Could not reorder certificates.', 'error');
    }
  };
